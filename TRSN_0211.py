...
class FSLModel(tf.keras.Model):
    def __init__(self, imageshape=(84, 84, 3), num_class=64, name=model_name):
        ...
        self.proto_attention_referenced_conv = Sequential([
            layers.Conv2D(1,
                          kernel_size=(1, 1),
                          strides=(1, 1),
                          padding="same",
        ...
        self.proto_attention_conv = Sequential([
            layers.Conv2D(1,
                          kernel_size=(3, 3),
                          padding="same",
                          kernel_regularizer=tf.keras.regularizers.l2(
        ...
        self.pooling3 = tf.keras.layers.AveragePooling2D(padding="same", name="pooling3", pool_size=(3, 3),
                                                         strides=1)
        self.pooling3.build([None, feature_size_h, feature_size_w, feature_dim])
        self.pooling5 = tf.keras.layers.AveragePooling2D(padding="same", name="pooling5", pool_size=(5, 5),
                                                         strides=1)
        self.pooling5.build([None, feature_size_h, feature_size_w, feature_dim])
        ...
        self.mean_query_acc_v2 = tf.keras.metrics.Mean(name="mq_acc_v2")
        self.mean_query_acc_attention = tf.keras.metrics.Mean(name="mq_acc_att")
    ...
    def spp(self, features, training=None):
        _, f_h, f_w, f_c = tf.unstack(tf.shape(features))
        f1 = features
        f3 = self.pooling3(features)
        f5 = self.pooling5(features)
        return tf.concat([f1, f3, f5], -1)
    ...
    def meta_train_step(self, support, query, training=None):
        ...
        attention_features_support = self.attention(support_features, training=training)
        ...
        attention_features_query = self.attention(query_features, training=training)
        ...
        support_logits_broad = tf.reshape(support_logits_mean, [batch, 1, ways, 1, 1, f_c])
        support_logits_broad = tf.broadcast_to(support_logits_broad, [batch, ways * query_shots, ways, f_h, f_w, f_c])
        ...
        loss_clc = tf.reduce_mean(
            tf.keras.losses.categorical_crossentropy(support_global_label, support_pred, from_logits=True))

        loss_clc = 0.5 * loss_clc + 0.5 * tf.reduce_mean(
            tf.keras.losses.categorical_crossentropy(support_global_label, weighted_local_logits_query_pred,
                                                     from_logits=True))
        ...
        #     tf.keras.losses.binary_crossentropy(pseudo_label, query_self_attention))
        salient_loss = tf.reduce_mean(
            tfa.losses.SigmoidFocalCrossEntropy()(pseudo_label, query_self_attention))

        query_features = tf.nn.l2_normalize(query_features, -1)
        query_logits_attention = query_features * logits_weights_query_referenced
        query_logits_attention = tf.math.divide_no_nan(tf.reduce_sum(query_logits_attention, [1, 2]),
                                                       tf.reduce_sum(logits_weights_query_referenced, [1, 2]))
        query_logits_attention = tf.nn.l2_normalize(query_logits_attention, -1)

        reshape_query_logits_base = tf.reshape(query_logits_attention, [batch, ways * query_shots, -1])
        reshape_query_logits_base = tf.nn.l2_normalize(reshape_query_logits_base, -1)
        sim = tf.linalg.matmul(reshape_query_logits_base, tf.stop_gradient(support_logits_mean), transpose_b=True)
        sim = tf.nn.softmax(sim / 10., -1)
        ...
        # total_loss = loss_clc + contrastive_neg_loss + meta_contrast_loss

        return [loss_clc, contrastive_neg_loss, salient_loss, meta_contrast_loss]
    ...
    def reset_metrics(self):
        ...
        self.mean_query_acc_v2.reset_states()
    ...
    def train_step_normal(self, data):
        ...
        with tf.GradientTape() as tape:
            total_loss = self.meta_train_step(support, query, True)
        loss_clc, contrastive_neg_loss, salient_loss, meta_contrast_loss = total_loss
        ...
        self.salient_loss_metric.update_state(meta_contrast_loss)
    ...
    def train_step_meta(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            sim = tf.linalg.matmul(reshape_query_logits, support_mean, transpose_b=True)
            ...
            dist_attention = tf.reshape(dist_attention, [batch, ways, query_shots, -1])
            dist_attention = tf.nn.softmax(dist_attention * 20, -1)
            meta_contrast_loss_attention = tf.keras.losses.categorical_crossentropy(
                tf.reshape(query_label, [-1, tf.shape(query_label)[-1]]),
                tf.reshape(dist_attention, [-1, tf.shape(dist_attention)[-1]]))
            meta_contrast_loss_attention = tf.reduce_mean(meta_contrast_loss_attention)
            ...
            dist_base = tf.reshape(dist_base, [batch, ways, query_shots, -1])
            dist_base = tf.nn.softmax(dist_base * 20, -1)
            meta_contrast_loss_base = tf.keras.losses.categorical_crossentropy(
                tf.reshape(query_label, [-1, tf.shape(query_label)[-1]]),
                tf.reshape(dist_base, [-1, tf.shape(dist_base)[-1]]))
            meta_contrast_loss_base = tf.reduce_mean(meta_contrast_loss_base)

            # avg_loss = meta_contrast_loss + meta_contrast_loss_attention + meta_contrast_loss_base
            avg_loss = meta_contrast_loss + meta_contrast_loss_attention + meta_contrast_loss_base
    ...
    def test_step_meta(self, data):
        ...
        support_logits_attention_reversed = support_features * (1. - support_self_attention)
        support_logits_attention_reversed = tf.math.divide_no_nan(
            tf.reduce_sum(support_logits_attention_reversed, [1, 2]),
            tf.reduce_sum((1. - support_self_attention), [1, 2]))
        support_logits_attention_reversed = tf.nn.l2_normalize(support_logits_attention_reversed, -1)

        support_logits_fusion_v2 = support_logits + support_logits_attention

        support_logits_fusion_v2 = tf.reshape(support_logits_fusion_v2,
                                              [batch, ways, shots, tf.shape(support_logits_fusion_v2)[-1]])
        support_logits_fusion_v2 = tf.nn.l2_normalize(support_logits_fusion_v2, -1)
        ...
        # self_logits_weights = tf.linalg.matmul(support_logits_fusion, support_logits_fusion, transpose_b=True)
        # self_logits_weights = 1. - tf.clip_by_value(self_logits_weights, 0., 1.) + tf.eye(shots)
        # self_logits_weights = tf.reduce_sum(self_logits_weights, -1, keepdims=True)
        # x_mean = tf.reduce_sum(support_logits_fusion * self_logits_weights, 2) / tf.reduce_sum(
        #     self_logits_weights, 2)
        x_mean_v2 = tf.reduce_mean(support_logits_fusion_v2, 2)
        x_mean = tf.reduce_mean(support_logits_fusion, 2)
        ...
        # self_logits_weights = tf.linalg.matmul(support_logits_attention, support_logits_attention, transpose_b=True)
        # self_logits_weights = 1. - tf.clip_by_value(self_logits_weights, 0., 1.) + tf.eye(shots)
        # self_logits_weights = tf.reduce_sum(self_logits_weights, -1, keepdims=True)
        # x_mean_attention = tf.reduce_sum(support_logits_attention * self_logits_weights, 2) / tf.reduce_sum(
        #     self_logits_weights, 2)
        ...
        query_logits_attention_reversed = query_features * (1. - query_self_attention)
        query_logits_attention_reversed = tf.math.divide_no_nan(tf.reduce_sum(query_logits_attention_reversed, [1, 2]),
                                                                tf.reduce_sum((1. - query_self_attention), [1, 2]))
        query_logits_attention_reversed = tf.nn.l2_normalize(query_logits_attention_reversed, -1)

        query_logits_fusion_v2 = query_logits + query_logits_attention
        ...
        logits_dim = tf.shape(x_mean_v2)[-1]

        support_mean_v2 = tf.reshape(x_mean_v2, [batch, ways, logits_dim])
        support_mean_v2 = tf.nn.l2_normalize(support_mean_v2, -1)
        reshape_query_logits_fusion_v2 = tf.reshape(query_logits_fusion_v2,
                                                    [batch, ways * query_shots, logits_dim])
        reshape_query_logits_fusion_v2 = tf.nn.l2_normalize(reshape_query_logits_fusion_v2, -1)
        dist_v2 = tf.linalg.matmul(reshape_query_logits_fusion_v2, support_mean_v2, transpose_b=True)
        query_pred_fusion_v2 = tf.clip_by_value(dist_v2, 0., 1.)

        acc_v2 = tf.keras.metrics.categorical_accuracy(tf.reshape(query_label, [batch, -1, dim_shape]),
                                                       query_pred_fusion_v2)
        acc_v2 = tf.reduce_mean(acc_v2, -1)
        self.mean_query_acc_v2.update_state(acc_v2)

        logs = {
            self.mean_query_acc.name: self.mean_query_acc.result(),
            self.mean_query_acc_v2.name: self.mean_query_acc_v2.result(),
