...
class FSLModel(tf.keras.Model):
    ...
    def train_step_normal(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            contrastive_loss = tfa.losses.npairs_loss(tf.argmax(y, -1), logits)

        trainable_vars = self.encoder.trainable_weights + self.clc.trainable_weights + self.projection.trainable_weights
        grads = tape.gradient([contrastive_loss], trainable_vars)
    ...
    def train_step_self_supervised(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            npair_logits = tf.matmul(
                feature_vectors_normalized, tf.transpose(feature_vectors_normalized)
            )
            # npair_logits = tf.divide(npair_logits, 0.05)

            contrastive_loss = tfa.losses.npairs_loss(tf.argmax(y, -1), npair_logits)
    ...
    def train_step_meta(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            sim = tf.nn.softmax(sim * self.T, -1)
...
if multi_gpu is True:
    ...
    with mirrored_strategy.scope():
        model = FSLModel(imageshape=(84, 84, 3), num_class=351)
else:
    model = FSLModel(imageshape=(84, 84, 3), num_class=351)
...
# model.fine_tune(weights="/data/giraffe/0_FSL/Normal_npairs_3_ckpts/model_e253-l 0.82962.h5", lr=0.005)
# model.fine_tune(weights="/data/giraffe/0_FSL/Normal_npairs_ckpts/model_e187-l 0.83129.h5", lr=0.005)
