...
def process_augment(x):
    ...
    return tf.stack([x, x_rotated_1, x_rotated_2, x_resized_1, x_resized_2], 0)
...
class ResetProjection(tf.keras.callbacks.Callback):
    ...
    def __init__(self):
        super(ResetProjection, self).__init__()

    def on_epoch_begin(self, epoch, logs=None):
        # Set the history attribute on the model after the epoch ends. This will
        # make sure that the state which is set is the latest one.
        temp_layer = tf.keras.Sequential([tf.keras.layers.Dense(128,
                                                                kernel_regularizer=tf.keras.regularizers.l2(
                                                                    WEIGHT_DECAY),
                                                                name="projection"),
                                          tf.keras.layers.LeakyReLU(0.2)])
        temp_layer.build([None, 640])
        tmp_weights = temp_layer.get_weights()
        tf.print("##Reset projection weights at e{}".format(epoch))
        self.model.projection.set_weights(tmp_weights)
...
class FSLModel(tf.keras.Model):
    ...
    def train_step_self_supervised(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            projections = self.projection(tf.reshape(logits, [-1, tf.shape(logits)[-1]]), training=training)
            projections = tf.reshape(projections, [5, -1, tf.shape(projections)[-1]])
            proto = tf.reduce_mean(projections, 0, keepdims=True)

            sim = tf.keras.losses.cosine_similarity(projections, proto, -1)
    ...
    def show(self, weights=None, data_dir_path="/data/giraffe/0_FSL/data/mini_imagenet_tools/processed_images_224",
        ...
        self.train_step = self.train_step_normal
        from datasets.dataloader import process_with_crop_with_contrastive
        ...
        for x, y in meta_test_ds:
            ...
            # support_image = tf.map_fn(process_augment, support_image)
            # support_image = tf.reshape(support_image, tf.concat([[-1], tf.shape(support_image)[-3:]], 0))
            # shots *= 5
            # support_label = tf.repeat(support_label, 5, 2)
        ...
        if contrastive is False:
            ...
            for x, y in meta_train_ds:
                ...
                proto = tf.stack([logits_base, logits_rotated_1, logits_rotated_2, logits_resized_1, logits_resized_2],
                                 0)
                means = tf.math.reduce_mean(proto, 0)

                logits = tf.concat([logits, means], 0)
                all_lables.extend(["P_{}".format(l) for l in y])
                self.show_TSNE(logits, all_lables)
...
# model.show("/data/giraffe/0_FSL/Normal__ckpts/model_e080-l 0.81660.h5")
# model.show("model_e273-l 0.82473.h5")
# model.show("/data2/giraffe/0_FSL/TRSN_2_ckpts/model_e054-l 0.85513.h5")
model.fine_tune(weights="/data/giraffe/0_FSL/Normal_centroid_ckpts/model_e119-l 0.81969.h5", lr=0.005)
...
# model.test(weights="/data/giraffe/0_FSL/Normal__ckpts/model_e080-l 0.81660.h5", shots=5, episode_num=600)
