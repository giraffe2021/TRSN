...
class FSLModel(tf.keras.Model):
    ...
    def train_step_meta(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            query_features_broad = tf.reshape(query_features, [batch, ways * query_shots, 1, f_h, f_w, f_c])
            query_features_broad = tf.broadcast_to(query_features_broad,
    ...
    def run(self, lr=0.001, weights=None, ways=5, shots=5, test_shots=15,
        ...
        train_batch = 4
        episode_num = 1200
        steps_per_epoch = 10 * episode_num // train_batch
        train_epoch = 20
        mix_up = True
        augment = True
        ...
        self.fit(meta_train_ds.repeat(), epochs=200,
...
if multi_gpu is True:
    ...
    with mirrored_strategy.scope():
        model = FSLModel(imageshape=(84, 84, 3), num_class=20)
else:
    model = FSLModel(imageshape=(84, 84, 3), num_class=20)

# model.fine_tune(weights="/data/giraffe/0_FSL/TRRN_2_ckpts/model_e273-l 0.82。473.h5",
#                 lr=0.001)
model.show(weights="/data2/giraffe/0_FSL/{}_ckpts/model_e197-l 0.84864.h5".format(model_name),
