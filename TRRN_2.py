...
class FSLModel(tf.keras.Model):
    def __init__(self, imageshape=(84, 84, 3), num_class=64, name=model_name):
        ...
        self.eroder = Sequential([
            ...
            layers.BatchNormalization(),
            layers.LeakyReLU(alpha=0.2),
            layers.Dense(128, kernel_regularizer=tf.keras.regularizers.l2(WEIGHT_DECAY)),
            layers.BatchNormalization(),
            layers.LeakyReLU(alpha=0.2),
        ],
            name="eroder")
        self.eroder.build([None, feature_dim * 2])
    ...
    def train_step_normal(self, data):
        ...
        y = tf.concat([y, y, y, y, y], 0)
        with tf.GradientTape() as tape:
            logits = self.gap(self.encoder(x, training=training))
            ...
            pred = self.clc(logits)
            clc_loss = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(y, pred, from_logits=True))
        ...
        grads = tape.gradient([clc_loss, rotate_loss, ratio_loss], trainable_vars)
        # grads = tape.gradient([clc_loss], trainable_vars)
        self.optimizer.apply_gradients(zip(grads, trainable_vars))

        # protos = tf.reduce_mean(support_logits, -2)
        # protos_broad = tf.repeat(tf.reshape(protos, [batch, 1, ways, dim_shape]), ways * query_shots, 1)
        # query_logits = tf.repeat(tf.reshape(query_logits, [batch, ways * query_shots, 1, dim_shape]), ways, 2)
        # x = tf.concat([protos_broad, query_logits], -1)
        # x = tf.reshape(x, [-1, dim_shape * 2])
        #
        # protos_broad_x = tf.repeat(tf.reshape(protos, [batch, 1, ways, dim_shape]), ways * query_shots, 1)
        # protos_broad_x = tf.stop_gradient(protos_broad_x)
        #
        # query_label = tf.reshape(query_label, [batch, ways * query_shots, -1])
        #
        # with tf.GradientTape() as tape:
        #     x_erode = self.eroder(x, training=training)
        #     # e_loss = tf.reduce_mean(
        #     #     tf.keras.losses.cosine_similarity(tf.reshape(protos_broad, [-1, dim_shape]), x_erode) ** 2)
        #     x_hat = self.decoder(x_erode, training=training)
        #     x_hat = tf.reshape(x_hat, [batch, ways * query_shots, ways, dim_shape])
        #
        #     recon_loss = -1. * tf.keras.losses.mse(tf.stop_gradient(protos_broad_x), x_hat)
        ...
        #     recon_loss = recon_loss + tf.reduce_mean(tf.reduce_sum(recon_loss * query_label, -1))
        # trainable_vars = self.eroder.trainable_weights + \
    ...
    def run(self, lr=0.001, weights=None, ways=5, shots=5, test_shots=15,
        ...
        meta_train_ds, steps_per_epoch = dataloader.get_all_dataset(phase='train', batch=64,
    ...
    def show(self, weights=None, data_dir_path="/data/giraffe/0_FSL/data/mini_imagenet_tools/processed_images_224",
        ...
        for x, y in meta_test_ds:
            ...
            # query_image = self.random_erode(query_image)
    ...
    def test_step_meta(self, data):
        ...
        protos = tf.reduce_mean(support_logits, -2)
        protos_broad = tf.repeat(tf.reshape(protos, [batch, 1, ways, dim_shape]), ways * query_shots, 1)
        query_logits_broad = tf.repeat(tf.reshape(query_logits, [batch, ways * query_shots, 1, dim_shape]), ways, 2)
        x = tf.concat([protos_broad, query_logits_broad], -1)
        x = tf.reshape(x, [-1, dim_shape * 2])
        x_erode = self.eroder(x, training=training)
        x_hat = self.decoder(x_erode, training=training)
        x_hat = tf.reshape(x_hat, [batch, ways * query_shots, ways, dim_shape])
...
if multi_gpu is True:
    ...
    with mirrored_strategy.scope():
        model = FSLModel(imageshape=(84, 84, 3), num_class=60)
else:
    model = FSLModel(imageshape=(84, 84, 3), num_class=60)
