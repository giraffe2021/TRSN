...
class FSLModel(tf.keras.Model):
    def __init__(self, imageshape=(84, 84, 3), num_class=64, name=model_name):
        ...
        self.projection = tf.keras.Sequential([tf.keras.layers.Dense(128,
    ...
    def train_step_normal(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            projection = self.projection(logits, training=training)
            feature_vectors_normalized = tf.math.l2_normalize(projection, axis=-1)
            y_true = tf.expand_dims(tf.argmax(y, -1), -1)
            y_true = tf.cast(tf.not_equal(y_true, tf.transpose(y_true)), projection.dtype)
            # Compute logits
            sim = tf.matmul(feature_vectors_normalized, feature_vectors_normalized, transpose_b=True)
            contrastive_loss = tf.math.divide_no_nan(tf.reduce_sum(y_true * (sim + 1)), tf.reduce_sum(y_true))
        trainable_vars = self.encoder.trainable_weights + self.clc.trainable_weights + self.projection.trainable_weights
        grads = tape.gradient([clc_loss, contrastive_loss], trainable_vars)
    ...
    def train_step_self_supervised(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            proto = tf.stack([logits_base, logits_rotated_1, logits_rotated_2, logits_resized_1, logits_resized_2], 0)
            proto = tf.reduce_mean(proto, 0)
            # proto_projection = self.projection(proto, training=training)
            proto_projection = proto

            feature_vectors_normalized = tf.math.l2_normalize(proto_projection, axis=-1)
            # Compute logits
            sim = tf.matmul(feature_vectors_normalized, feature_vectors_normalized, transpose_b=True)
            # sim = tf.matmul(feature_vectors_normalized, feature_vectors_normalized, transpose_b=True)**2
            # off_diag = self.get_off_diag(sim)
            # contrastive_loss = tf.reduce_mean(off_diag)

            y_true = tf.expand_dims(tf.argmax(y, -1), -1)
            y_true = tf.cast(tf.not_equal(y_true, tf.transpose(y_true)), proto.dtype)
            # contrastive_loss = tf.math.divide_no_nan(tf.reduce_sum(y_true * tf.clip_by_value(sim, 0, 1)),
            #                                   tf.reduce_sum(y_true))

            sim = y_true * tf.clip_by_value(sim, 0, 1)
            contrastive_loss = tf.math.divide_no_nan(tf.reduce_sum(sim),
                                                     tf.reduce_sum(tf.cast(tf.not_equal(sim, 0), tf.float32))) ** 2
    ...
    def run(self, lr=0.001, weights=None, ways=5, shots=5, test_shots=15,
        ...
        self.fit(meta_train_ds, epochs=total_epoch,
                 ...
                 callbacks=callbacks, initial_epoch=78)
...
if multi_gpu is True:
    ...
    model = FSLModel(imageshape=(84, 84, 3), num_class=64)
# model.test(weights="/data/giraffe/0_FSL/Normal_ckpts/latest.h5")
# model.run(weights="/data/giraffe/0_FSL/Normal_barrow_ckpts/model_e078-l 0.81184.h5")
# model.show("/data2/giraffe/0_FSL/Normal_ckpts/model_e174-l 0.46830.h5")
# model.show("model_e273-l 0.82473.h5")
# model.show("/data2/giraffe/0_FSL/TRSN_2_ckpts/model_e054-l 0.85513.h5")
model.fine_tune(weights="/data/giraffe/0_FSL/Normal_barrow_ckpts/model_e078-l 0.81184.h5", lr=0.005)
