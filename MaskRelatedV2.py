...
class FSLModel(tf.keras.Model):
    def __init__(self, imageshape=(84, 84, 3), num_class=64, name=model_name):
        ...
        self.local_cov = Sequential([
            layers.Conv2D(self.num_class,
                          ...
                          name="local_cov"),
            layers.BatchNormalization(),
            layers.LeakyReLU(alpha=0.2)], name="local_cov")
        self.local_cov.build([None, feature_size_h, feature_size_w, feature_dim])
    ...
    def calculate_entropy_v1(self, support_mean_base, query_logits, temper=20.):
    ...
    def calculate_entropy(self, support_mean_base, query_logits, temper=1.):
        batch = tf.shape(query_logits)[0]
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            support_local_features = self.local_cov(support_features, training=training)
            query_features = self.encoder(query_x, training=training)
            query_local_features = self.local_cov(query_features, training=training)
            ...
            support_features = tf.reshape(support_features, [batch, ways, shots, f_h, f_w, f_c])
            begin_end = tf.random.shuffle(tf.range(shots + 1))[:2]
            begin, end = tf.reduce_min(begin_end), tf.reduce_max(begin_end)
            sampled_k = end - begin
            reshape_sampled_features = tf.reshape(support_features[..., begin:end, :, :, :],
                                                  [batch, ways, sampled_k * f_h * f_w, f_c])
            reshape_sampled_features = tf.nn.l2_normalize(reshape_sampled_features, -1)
            support_features = tf.reshape(support_features,
                                          [batch, ways, shots * f_h * f_w, f_c])
            l2_support_features = tf.nn.l2_normalize(support_features, -1)
            query_features = tf.reshape(query_features,
                                        [batch, ways, query_shots * f_h * f_w, f_c])
            l2_query_features = tf.nn.l2_normalize(query_features, -1)
            self_similarity = tf.linalg.matmul(l2_support_features, reshape_sampled_features, transpose_b=True)
            self_similarity = tf.clip_by_value(self_similarity, 0., 1.)
            self_similarity = tf.reduce_mean(self_similarity, -1)
            self_similarity = tf.reshape(self_similarity, [-1, f_h, f_w, 1])
            cross_similarity = tf.linalg.matmul(l2_query_features, reshape_sampled_features, transpose_b=True)
            cross_similarity = tf.clip_by_value(cross_similarity, 0., 1.)
            cross_similarity = tf.reduce_mean(cross_similarity, -1)
            cross_similarity = tf.reshape(cross_similarity, [-1, f_h, f_w, 1])
            support_refereced_base = tf.math.divide_no_nan(
                tf.reduce_sum(support_local_features * self_similarity, [-3, -2]),
                tf.reduce_sum(self_similarity, [-3, -2]))
            query_referenced_base = tf.math.divide_no_nan(
                tf.reduce_sum(query_local_features * cross_similarity, [-3, -2]),
                tf.reduce_sum(cross_similarity, [-3, -2]))
            pred_referenced = tf.concat([support_refereced_base, query_referenced_base], 0)
            pred_referenced = self.bn(pred_referenced)
            loss_clc = loss_clc * 0.5 + 0.5 * tf.reduce_mean(
                tf.keras.losses.categorical_crossentropy(y, pred_referenced, from_logits=True))
        ...
        #     query_logits = tf.reshape(query_logits, [batch, ways, query_shots, -1])
        #     query_logits = tf.nn.l2_normalize(query_logits, -1)
        #     support_mean_base = tf.nn.l2_normalize(support_refereced_base, -1)
        #
        ...
        #         transpose_b=True)
        #     sim_map = tf.reshape(sim_map, [batch, ways * query_shots, ways, *feature_shape[:-1], 1])
        #     mask = tf.cast(tf.less_equal(sim_map, self.thresh_min), tf.float32)
        #     enhanced_mask = tf.cast(tf.greater_equal(sim_map, self.thresh_max), tf.float32)
        #
        #     max_enhanced_mask = tf.cast(tf.greater_equal(tf.reduce_max(sim_map, 2), self.thresh_max), tf.float32)
        #     random_mask = max_enhanced_mask * tf.random.uniform(tf.shape(max_enhanced_mask), 0.0, 1.0)
        #     random_mask = tf.reshape(random_mask, [-1, *feature_shape[:-1], 1])
        #     random_enhanced_logits = self.gap(random_mask * query_features)
        #     random_enhanced_pred = self.clc(random_enhanced_logits)
        #     loss_clc = loss_clc * 0.5 + 0.5 * tf.reduce_mean(
        #         tf.keras.losses.categorical_crossentropy(query_global_label, random_enhanced_pred, from_logits=True))
        #
        ...
        #     enhanced_logits = self.gap(enhanced_query_feature)
        ...
        #     entropy_diff = tf.nn.softmax(entropy_diff * 50., -1)
        #
    ...
    def show(self, weights=None, data_dir_path="/data/giraffe/0_FSL/data/mini_imagenet_tools/processed_images_224",
        ...
        for x, y in meta_test_ds:
            ...
            _, logits_dim = tf.unstack(tf.shape(support_logits))
            _, f_h, f_w, f_c = tf.unstack(tf.shape(support_features))
            reshape_support_features = tf.reshape(support_features,
                                                  [batch, ways, shots * f_h * f_w, f_c])
            reshape_support_features = tf.nn.l2_normalize(reshape_support_features, -1)
            self_similarity = tf.linalg.matmul(reshape_support_features, reshape_support_features, transpose_b=True)
            self_similarity = tf.reduce_sum(self_similarity, -1)
            self_similarity = tf.reshape(self_similarity, [batch, ways, shots * f_h * f_w, 1])

            support_mean_base = tf.math.divide_no_nan(tf.reduce_sum(reshape_support_features, -2),
                                                      tf.reduce_sum(self_similarity, -2))
            query_logits = tf.reshape(query_logits, [batch, ways, query_shots, -1])
            query_logits = tf.nn.l2_normalize(query_logits, -1)
            support_mean_base = tf.nn.l2_normalize(support_mean_base, -1)
            sim, entropy = self.calculate_entropy(support_mean_base, query_logits)
...
model.test("/data2/giraffe/0_FSL/{}_ckpts/latest.h5".format(model_name), shots=5)
